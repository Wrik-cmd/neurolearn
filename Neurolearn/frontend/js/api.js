...
const API = (() => {
  ...
  async function request(method, endpoint, body = null) {
    ...
    if (token && token.startsWith('demo-token-')) {
       const isPublicTarget = method === 'GET' && (endpoint.startsWith('/courses') || endpoint.includes('/questions'));
       
       if (!isPublicTarget) {
           await new Promise(r => setTimeout(r, 200)); // small delay
           let mData = [];
           if (endpoint === '/dashboard/stats') mData = mock.stats;
           else if (endpoint === '/dashboard/activity') mData = mock.activity;
           else if (endpoint === '/dashboard/weak-topics') mData = mock.weakTopics;
           else if (endpoint === '/dashboard/chart') mData = mock.chartData;
           else if (endpoint === '/recommendation') mData = mock.recommendations;
           else if (endpoint === '/profile') mData = mock.user;
           else if (endpoint === '/quiz/history') mData = [];
           else mData = { message: 'Mock success' };
           
           return { success: true, data: mData };
       }
    }
    ...
    try {
      ...
      if (err.message.includes('Token is not valid') || err.message.includes('authorization denied')) {

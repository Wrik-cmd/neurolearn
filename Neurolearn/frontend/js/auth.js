...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (isLoginPage) {
    ...
    function mockLogin(email) {
      // Demo: store mock user and redirect
      localStorage.setItem('nl_token', 'demo-token-' + Date.now());
      localStorage.setItem('nl_user', JSON.stringify({ ...API.mock.user, email }));
      showSuccess(btn, 'ACCESS GRANTED');
      setTimeout(() => { window.location.href = '/dashboard.html'; }, 800);
    }
  }
  ...
  if (isSignupPage) {
    ...
    function mockSignup(name, email) {
      ...
      localStorage.setItem('nl_token', 'demo-token-' + Date.now());
      localStorage.setItem('nl_user', JSON.stringify({ id: 1, name, email, xp: 0, level: 1, avatar: initials }));
      showSuccess(btn, 'WELCOME ABOARD!');
      setTimeout(() => { window.location.href = '/dashboard.html'; }, 800);
    }
  }
